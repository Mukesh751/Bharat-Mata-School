    ...
    if(move_uploaded_file($_FILES['profile_pic']["tmp_name"], $directoery. $_FILES['profile_pic']['name']))
{
    ...
    {
        $sql = mysqli_query($conn,"INSERT INTO studentdata (fullname, admissionnumber, dob, gender, fathername, fatheroccupation, mothername, motheroccupation, email, phonenumber, fulladdress, class, photo,username, passcode, teacherId) VALUES 
        ('$name', '$admissionnumber', '$dob', '$gender', '$fathername', '$fatheroccupation', '$mothername', '$motheroccupation', '$email', '$phonenumber', '$fulladdress', '$class', '$photo','$username','$password','$teacherId')");
