<div class="pt-3">
    <!-- Footer -->
    <footer
            class="text-center text-lg-start text-white"
            style="background-color: #fdd835; "
            >
      <!-- Grid container -->
      <div class=" p-4 pb-0">
        <!-- Section: Links -->
        <section class="">
          <!--Grid row-->
          <div class="row">
            <!-- Grid column -->
            <div class="col-md-3 col-lg-3 col-xl-3  text-dark">
              <img src="images/logo-removebg-preview.png" class="img-fluid">
            
              <p>
                We educate the children through a comprehensive accdemic curriculam, which provides perfect place for students to mind their talents in an unfettered manner to their optical degree of performance.
              </p>
            </div>
            <!-- Grid column -->
  
            <hr class="w-100 clearfix d-md-none" />
  
            <!-- Grid column -->
            <div class="col-md-2 col-lg-2 col-xl-2 mx-auto mt-3 text-dark">
              <h6 class="text-uppercase mb-4 font-weight-bold">Useful Links</h6>
              <p>
                <a href="afterlogindex.php" style = "color: black;">Home</a>
              </p>
              <p>
                <a href="aboutschool.php" style = "color: black;">About School</a>
              </p>
              <p>
                <a href="faculty.php" style = "color: black;">Faculty</a>
              </p>
              <p>
                <a href="courses.php" style = "color: black;">Courses</a>
              </p>
              <p>
                <a href="imagegallery.php" style = "color: black;">Gallery</a>
              </p>
              <p>
                <a href="events.php" style = "color: black;">Events</a>
              </p>
              <p>
                <a href="contact.php" style = "color: black;">Contact</a>
              </p>
            </div>
            <!-- Grid column -->
  
            <hr class="w-100 clearfix d-md-none" />
  
            <!-- Grid column -->
            <hr class="w-100 clearfix d-md-none" />
  
            <!-- Grid column -->
            <div class="col-md-4 col-lg-3 col-xl-3 mx-auto mt-3 text-dark">
              <h6 class="text-uppercase mb-4 font-weight-bold">Contact</h6>
              <p><a style = "color: black;" href="https://goo.gl/maps/k3SD5JjMfevfXmh18" target="_blank"><i class="fa fa-home mr-3"></i>Rakhi, Sec.26, Naya Raipur, raipur, Chhattisgarh 492015</a></p>
              <p><a style = "color: black;" href="mailto:bharatmataschool@gmail.com" target="_blank"><i class="fa fa-envelope mr-3"></i> bharatmataschool@gmail.com</a></p>
              <p><a style = "color: black;" href="tel:+91 xxxxxx xxxxx"><i class="fa fa-phone mr-3"></i> +91 094791 18247</a></p>
              
            </div>
            <!-- Grid column -->
  
            <!-- Grid column -->
            <div class="col-md-3 col-lg-2 col-xl-2 mx-auto mt-3">
              <h6 class="text-uppercase mb-4 font-weight-bold text-dark">Follow us</h6>
  
              <!-- Facebook -->
              <a
                 class="btn btn-primary btn-floating m-1"
                 style="background-color: #3b5998"
                 href="#!"
                 role="button"
                 ><i class="fa fa-facebook-f"></i
                ></a>
  
              <!-- Twitter -->
              <a
                 class="btn btn-primary btn-floating m-1"
                 style="background-color: #55acee"
                 href="#!"
                 role="button"
                 ><i class="fa fa-twitter" aria-hidden="true"></i>
                </a>
  
              <!-- Google -->
              <a
                 class="btn btn-primary btn-floating m-1"
                 style="background-color: #dd4b39"
                 href="#!"
                 role="button"
                 ><i class="fa fa-google"></i
                ></a>
  
              <!-- Instagram -->
              <a
                 class="btn btn-primary btn-floating m-1"
                 style="background-color: #ac2bac"
                 href="#!"
                 role="button"
                 ><i class="fa fa-instagram"></i
                ></a>
  
              <!-- Linkedin -->
              <a
                 class="btn btn-primary btn-floating m-1"
                 style="background-color: #0082ca"
                 href="#!"
                 role="button"
                 ><i class="fa fa-linkedin" aria-hidden="true"></i>
                </a>
              <!-- Github -->
              <a
                 class="btn btn-primary btn-floating m-1"
                 style="background-color: #333333"
                 href="#!"
                 role="button"
                 ><i class="fa fa-github"></i
                ></a>
            </div>
          </div>
          <!--Grid row-->
        </section>
        <!-- Section: Links -->
      </div>
      <!-- Grid container -->
  
      <!-- Copyright -->
      <div
           class="text-center p-3"
           style="color:#000000"
           >
        © 2022 Copyright:
        <a class="text-dark" href="#"
           >Bharat Mata School</a
          >
      </div>
      <!-- Copyright -->
    </footer>
    <!-- Footer -->
  </div>
